-- CHECK constraints, Full-Text Search et indexes stratégiques
-- À appliquer après la migration Prisma initiale
...
ALTER TABLE recipes ADD CONSTRAINT ck_recipes_prep_time
  CHECK (prep_time_minutes >= 0);
ALTER TABLE recipes ADD CONSTRAINT ck_recipes_cook_time
  CHECK (cook_time_minutes >= 0);
ALTER TABLE recipes ADD CONSTRAINT ck_recipes_servings
  CHECK (servings >= 1);
...
-- Ingredients
ALTER TABLE ingredients ADD CONSTRAINT ck_ingredients_quantity
  CHECK (quantity > 0);
...
ALTER TABLE moderation_actions ADD CONSTRAINT ck_moderation_action_target_coherence
  CHECK (
...
CREATE INDEX IF NOT EXISTS idx_comments_recipe_active_idx ON comments(recipe_id) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_recipe_views_chrono
  ON recipe_views(viewed_at DESC);
